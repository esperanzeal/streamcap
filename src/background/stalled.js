...
// 等 content 就绪（每 500ms 探一次 PING，共 tries 次）
...
async function reloadTaskTab(d, reason) {
  ...
  // ★★ 方案 A：用 tabs.update({url}) 做**一次全新的 GET 导航**，而不是 tabs.reload()。
  //   真机现象：reload 的语义是"重新执行当前文档的加载" —— 若该文档由 POST 表单产生
  //   （有些站点"点播放"其实是提交表单），Chrome 会弹出「确认重新提交表单」；用户点继续 →
  //   服务器对该 URL 返回 405 → 页面变错误页 → content 不再注入 → 任务被标失败。
  //   用户手动按 F5 不报错，说明那次刷新时页面已是 GET 文档；显式导航与手动刷新等效，
  //   且对 GET/POST 两种状态都安全。
  ...
  try {
    ...
    ready = await waitContent(tabId, 40); // ≤20s
  } catch (e) {
    // 兜底：导航/轮询期间抛错（tab 恰在此刻被关闭、SW 时间片被打断）不能让任务停在中间态
    //   ——状态 queued 却不在队列 = 不占槽、不调度、无日志。放回队列，下一轮 alarm 再试。
  ...
  //   下载其实只要求"同一个 origin 的页面"（OPFS 按 origin 隔离、Referer 是 origin 级），
  //   不需要正好是那个视频页 —— 所以站点首页足够承载。
  if (!ready && navUrl) {
